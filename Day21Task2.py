...
# for larger values (18 already) this gets quite slow and the active cells blow up; I might be able to speed it up by 
# using memoization and storing now computed transformations as well; but that would mean I had to rewrite essentially the
# whole program to check whether that is actually applicable and which rule I can use (since the fact we lose information
# about the previous subsquare orientation and the fact that now we have not just sizes 2 and 3 of subsquares complicate matters
# heavily). Rewriting this would take more time than just running the program as is. 
